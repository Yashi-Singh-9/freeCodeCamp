...
1. The `background-color` property of the `.heart::after` selector should be `pink`.
2. The `border-radius` of the `.heart::after` selector should be 50%.
3. The `transform` property for the `heart` class should use a `rotate()` function set to -45 degrees.
4. The `content` of the `.heart::before` selector should be an empty string.
